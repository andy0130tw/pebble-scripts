...
def fix_ResourcePack_bug(system):
    spec = inspect.getfullargspec(ResourcePack.__init__)
...
def cmd_unpack(args):
    with open(args.pack_file, 'rb') as pack_file:
        pack = ResourcePack.deserialize(pack_file, is_system=args.system)
        print('num_files', pack.num_files)
        print('timestamp', pack.timestamp)
        for i in range(len(pack.table_entries)):
            ent = pack.table_entries[i]
            print(f'#{i:3d} -> {ent.content_index:03d} len={ent.length}')
